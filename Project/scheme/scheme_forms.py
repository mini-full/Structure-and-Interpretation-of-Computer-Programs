...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        env.define(signature.first, do_lambda_form(Pair(formals, body), env))
...
def do_if_form(expressions, env):
    ...
    if is_scheme_true(scheme_eval(expressions.first, env)):
        return scheme_eval(expressions.rest.first, env, True)           # EC
    elif len(expressions) == 3:
        return scheme_eval(expressions.rest.rest.first, env, True)      # EC
...
def do_define_macro(expressions, env):
    ...
    if isinstance(signature, Pair) and scheme_symbolp(signature.first):
        ...
        validate_formals(formals)
        body = expressions.rest
        env.define(signature.first, MacroProcedure(
            formals, body, env))  # env not necessary
        return signature.first
    else:
