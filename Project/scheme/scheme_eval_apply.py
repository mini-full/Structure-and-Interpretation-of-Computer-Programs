...
def scheme_eval(expr, env, _=None):  # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in scheme_forms.SPECIAL_FORMS:
        ...
        if isinstance(scheme_eval(first, env), MacroProcedure):
            macro = scheme_eval(first, env)
            parent = env
            child = parent.make_child_frame(macro.formals, expr.rest)
            tmp = eval_all_macro(macro.body, child)
            return scheme_eval(tmp, child)
...
def scheme_apply(procedure, args, env):
    ...
    if isinstance(procedure, BuiltinProcedure):
        ...
        # BEGIN PROBLEM 9
        parent = procedure.env
        child = parent.make_child_frame(procedure.formals, args)
...
def eval_all_macro(expressions, env):
