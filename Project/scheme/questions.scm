...
(define (enumerate s)
  ...
  ; ()
  ; BEGIN PROBLEM 15
  (define (enumerator lst index)
    (if (equal? lst '())
        lst
        (cons (list index (car lst))
              (enumerator (cdr lst) (+ 1 index)))))
  (enumerator s 0))
...
(define (merge inorder? list1 list2)
  ...
  ; BEGIN PROBLEM 16
  (cond 
    ((and (equal? list1 nil) (equal? list2 nil))
     '())
    ((and (not (equal? list1 nil)) (equal? list2 nil))
     list1)
    ((and (not (equal? list2 nil)) (equal? list1 nil))
     list2)
    (else
