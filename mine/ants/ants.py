...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8
            if place.ant.can_contain(self) and not self.is_container:
                place.ant.store_ant(self)
            elif self.can_contain(place.ant) and not place.ant.is_container:
                self.store_ant(place.ant)
                place.ant = self #其实没错，Pylance认为上面的store_ant()是一定会产生错误的，其实不然。
            else:
    ...
    def buff(self):
        ...
        # BEGIN Problem 12
        if hasattr(self, 'buffed') and not self.buffed:
            self.buffed = True
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        def helper(place, index):
            if place.is_hive or index > self.max_range:
                return None
            elif place.bees and index >= self.min_range:
                return random_bee(place.bees)
            else:
                return helper(place.entrance, index + 1)
    ...
    def nearest_bee(self):
        ...
        """
        # BEGIN Problem 3 and 4
        place, index = self.place, 0
        while not place.is_hive:
            bee = random_bee(place.bees)
            if bee and self.min_range <= index <= self.max_range:
                return bee
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        cpylst = list(self.place.bees)
        for everybee in cpylst:
            Ant.reduce_health(everybee, total_damage)
...
class HungryAnt(Ant):
    ...
    def __init__(self, health=1):
        """Create an Ant with a HEALTH quantity, 
        also initialize its chew_countdown to 0."""
        super().__init__(health)
    ...
    def action(self, gamestate):
        if self.chew_countdown:
            ...
            poorbee = random_bee(self.place.bees)
            if poorbee:
                poorbee.reduce_health(poorbee.health)
...
class TankAnt(ContainerAnt):
    """TankAnt provides protection to other Ants,
     and deals damage to the bees in its place."""
    ...
    def action(self, gamestate):
        ...
        cpylst = list(self.place.bees)
        for each_bee in cpylst:
            Insect.reduce_health(each_bee, self.damage)
# END Problem 9
...
# END Problem 12
    ...
    def construct(cls, gamestate):
        ...
        """
        # BEGIN Problem 12
        if hasattr(gamestate, 'queen'):
            return
        else:
            gamestate.queen = super().construct(gamestate)
    ...
    def action(self, gamestate):
        ...
        while curr_place.exit:
            ...
            if type(curr_place.ant) is BodyguardAnt\
                or type(curr_place.ant) is TankAnt:
                Ant.buff(curr_place.ant.ant_contained)
            Ant.buff(curr_place.ant)
        # END Problem 12
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        back_away = self.place.entrance
        # Extra credit: Special handling for bee direction
        if self.slowed:
            if gamestate.time % 2 == 0:
                if self.health > 0:
                    if self.scared and not back_away.is_hive:
                        self.move_to(back_away)
                        self.scared -= 1
                    elif not self.scared and destination is not None:
...
class NinjaAnt(Ant):
    ...
    def action(self, gamestate):
        # BEGIN Problem Optional 1
        cpylst = list(self.place.bees)
        for eachbee in cpylst:
            eachbee.reduce_health(self.damage)
