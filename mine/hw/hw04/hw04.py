...
def balanced(m):
    ...
    if is_planet(m):
        ...
        assert is_mobile(m), "m must be a mobile or a planet"
        leftarm = left(m)
        rightarm = right(m)
...
def replace_loki_at_leaf(t, lokis_replacement):
    ...
    if is_leaf(t):
        ...
        brans = [replace_loki_at_leaf(branche, lokis_replacement) for branche in branches(t)]
        return tree(label(t), brans)
...
def div_interval(x, y):
    ...
    reciprocal of y."""
    "*** YOUR CODE HERE ***"
    assert lower_bound(y) > 0 or 0 >= upper_bound(y), "y must not contain 0"
