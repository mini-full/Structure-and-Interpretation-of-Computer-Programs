...
def more_boar(player_score, opponent_score):
    ...
    digits1 = count_digits(player_score)
    digits2 = count_digits(opponent_score)
    if digits1 > 2:
        player_score //= (10 ** (digits1 - 2) )
    if digits2 > 2:
        opponent_score //= (10 ** (digits1 - 2) )
    # 递归为只有前两位 但个位数未判断
    if digits2==1:
        ret = False
    elif digits1==1 and digits2!=1:
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if not who:
            ...
            score0 += take_turn(n, score1 ,dice, goal)
            if not more_boar(score0, score1):
                who = next_player(who)
        else:
            n = strategy1(score1, score0)
            score1 += take_turn(n, score0, dice, goal)
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        if who:
            delta_score = score1 - last_score
            score = score1
        else:
            delta_score = score0 - last_score
            score = score0

        if delta_score > running_high:
            print('Player', who, 'has reached a new maximum point gain.', delta_score, 'point(s)!')
            return announce_highest(who, score, delta_score)
...
def make_averaged(original_function, trials_count=100000):
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000000):
...
def more_boar_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    score_after0 = score + piggy_points(opponent_score)
    if more_boar(score_after0, opponent_score):
        return 0
    else:
