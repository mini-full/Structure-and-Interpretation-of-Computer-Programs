...
def about(topic):
    ...
    def isabout(paragraph):
        lstOfWords = split(remove_punctuation(paragraph))
        for Topic in topic:
...
def accuracy(typed, reference):
    ...
    lengthrefer = len(reference_words)
    if length == 0 and lengthrefer == 0:
        return 100.0
    elif length == 0 and lengthrefer > 0:
        return 0.0
    elif lengthrefer == 0 and length > 0:
        return 0.0
    for i in range(min(lengthrefer, length)):
        word_typed = typed_words[i]
        word_refer = reference_words[i]
        # if len(word_refer) == len(word_typed):
        #     for i in range(len(word_typed)):
        #         if word_typed[i] != word_refer[i]:
        #             same = 0
        #     same = 1
        if word_typed == word_refer:
            same = 1
        else:
            same = 0
        same_cnt += same
    return 100 * same_cnt / length
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    for word in valid_words:
        if word == typed_word:
            return word
    mindiff = min(valid_words,key= lambda word: diff_function(typed_word, word, limit))
    if diff_function(typed_word, mindiff, limit) > limit:
        return typed_word
    else:
        return mindiff
    # END PROBLEM 5
...
def feline_flips(start, goal, limit):
    ...
    def helper(start, goal,  ret):
        startlen = len(start)
        goallen = len(goal)
        # if startlen == goallen == 1:
        #     if start == goal:
        #         return 0
        #     else:
        #         return 1
        if startlen == 0 or goallen == 0:
            return abs(startlen - goallen) + ret
        if ret > limit:
            return ret
        ret += 1 if start[0] != goal[0] else 0
        return helper(start[1:], goal[1:], ret)
...
def minimum_mewtations(start, goal, limit):
    ...
    """
    # assert False, 'Remove this line'
    def minmove(s, g, change):
        if change > limit:
            return change
        len_s = len(s)
        len_g = len(g)
        if len_s == 0 or len_g == 0:
            return max(len_s, len_g) + change 
        elif s[0] == g[0]:
            return minmove(s[1:], g[1:], change)
        else:
            removing = minmove(s[1:], g, change + 1)
            adding = minmove(s, g[1:], change + 1)
            substituting = minmove(s[1:], g[1:], change + 1)
            return min(removing, adding, substituting)
    return minmove(start, goal, 0)
...
def report_progress(sofar, prompt, user_id, upload):
    ...
    same = 1
    for cnt in range(len_sofar):
        if sofar[cnt] != prompt[cnt]:
            same = 0
            break
    if same:
        cnt = len_sofar
    progress = cnt / len_prompt
...
def time_per_word(words, times_per_player):
    ...
    # BEGIN PROBLEM 9
    time = []
    for timelst in times_per_player:
        ret = []
        for i in range(1, len(timelst)):
            ret += [timelst[i] - timelst[i-1]]
        time += [ret]
