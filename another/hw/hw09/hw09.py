...
def cs_classes(post):
    ...
    True
    """
    return bool(re.search(r"[Cc][Ss]\s*\d+[abcABC]?", post))
...
def match_time(text):
    ...
    ['2:00']
    """
    return re.findall(r"\s((?:(?:2[0-3])|(?:[01]?[0-9])):\d\d[AaPp]?[mM]?)", text)
...
def most_common_code(text):
    ...
    '123'
    """
    area_codes_lst = area_codes(text)
    # print(area_codes_lst)
    max = 0
    most_code = ""
    for area_code in area_codes_lst:
        if list.count(area_codes_lst, area_code) > max:
            max = list.count(area_codes_lst, area_code)
            most_code = area_code
    return most_code
