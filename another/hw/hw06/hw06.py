...
class VendingMachine:
    ...
    def restock(self, amount):
        self.stock += amount
        return 'Current ' + self.name + f' stock: {self.stock}'
...
def two_list(vals, amounts):
    ...
    def helper(cnt, index):
        if not cnt:
            ...
            return Link(vals[index], helper(cnt - 1, index))
...
class VirFib():
    ...
    def next(self):
        if self.value == 0:
            ...
            ret = VirFib(self.value + self.prev)
            ret.prev = self.value
            return ret
...
def bst_max(t):
    """Return the max label of a tree t."""
    if t.is_leaf():
        return t.label
    else:
        max_in_branches = max([bst_max(b) for b in t.branches])
        return max(t.label, max_in_branches)

def bst_min(t):
    """Return the min label of a tree t."""
    if t.is_leaf():
        return t.label
    else:
        min_in_branches = min([bst_min(b) for b in t.branches])
        return min(t.label, min_in_branches)
