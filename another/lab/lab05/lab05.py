...
def flatten(s):
    ...
    def helper(lst, ret):
        if not lst:
            ...
            return helper(lst[1:], ret + [lst[0]])
        else:
            ret += helper(lst[0], [])
            return helper(lst[1:], ret)
    return helper(s, [])
...
from cs61a.Lab.lab05.lab05 import branches
...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    dest = make_city('dest', lat, lon)
    distancea = distance(dest, city_a)
    distanceb = distance(dest, city_b)
...
def sprout_leaves(t, leaves):
    ...
    """
    if is_leaf(t):
        t = tree(t, [tree(leave) for leave in leaves])
    else:
        t = tree(label(t), [sprout_leaves(tr, leaves) for tr in branches(t)])
    return t
